...
class Decoder(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x5):
        ...
        #print(x.shape)
        #print(self.attention(x4).shape)
        x = torch.cat((x, self.attention(x4)), dim=1)
        x = self.relu(self.norm(self.conv2(x)))
        x = torch.cat((x, self.attention1(x3)), dim=1)
        x = self.relu(self.norm(self.conv3(x)))
        x = torch.cat((x, self.attention2(x2)), dim=1)
        x = self.relu(self.norm(self.conv4(x)))
        x = torch.cat((x, self.attention3(x1)), dim=1)
